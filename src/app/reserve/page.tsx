...
export default function ReservePage() {
  ...
  return (
    <main className="max-w-xl mx-auto p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Reserve a unit</h1>
      {unitError ? <p className="text-sm text-red-600">{unitError}</p> : null}
      {unit ? (
        <section className="rounded-md border p-4 text-sm space-y-1">
          <div><strong>Unit:</strong> {unit.name ?? unit.id}</div>
          {unit.type ? <div><strong>Type:</strong> {unit.type}</div> : null}
          {unit.width != null && unit.length != null ? (
            <div><strong>Size:</strong> {unit.width}&times;{unit.length}{unit.area != null ? " (" + unit.area + " sq ft)" : ""}</div>
          ) : null}
          {unit.rate != null ? <div><strong>Rate:</strong> {"$" + unit.rate.toFixed(2)} / month</div> : null}
        </section>
      ) : !unitError ? <p className="text-sm opacity-70">Loading unit…</p> : null}

      {submitted && result ? (
        <section className="rounded-md border p-4 space-y-2">
          <p className="font-medium">Reservation confirmed.</p>
          <p className="text-sm">Confirmation #: <span className="font-mono">{result.reservationId}</span></p>
          {result.accessCode ? (
            <p className="text-sm">Gate code: <span className="font-mono">{result.accessCode}</span></p>
          ) : null}
          <p className="text-sm pt-2">
            Ready to complete the rental?{" "}
            <a className="underline" href={rentHref}>Continue to checkout</a>
          </p>
        </section>
      ) : (
